...
def hbond_acceptor_donor(mol1, mol2, cutoff=3.5, tolerance=120):
    ...
    if len(a) > 0 and len(d) > 0:
        ...
        strict = (angle1 > tolerance).any(axis=-1)
        return a, d, strict
    else:
        return a, d, np.array([], dtype=bool)


def hbonds(mol1, mol2, cutoff=3.5, tolerance=120):
    ...
    strict : numpy array, dtype=bool
        ...
        only distance cutoff is met, therefore the bond is 'crude'.
    """
    a1, d1, s1 = hbond_acceptor_donor(
        mol1, mol2, cutoff=cutoff, tolerance=tolerance)
    a2, d2, s2 = hbond_acceptor_donor(
        mol2, mol1, cutoff=cutoff, tolerance=tolerance)
...
def halogenbonds(mol1, mol2, cutoff=4, tolerance=30):
    ...
    strict : numpy array, dtype=bool
        ...
        only distance cutoff is met, therefore the bond is 'crude'.
    """
    a1, h1, s1 = halogenbond_acceptor_halogen(
        mol1, mol2, cutoff=cutoff, tolerance=tolerance)
    a2, h2, s2 = halogenbond_acceptor_halogen(
        mol2, mol1, cutoff=cutoff, tolerance=tolerance)
